    ...
    GovernorVotesQuorumFraction
{
    ...
    ) external onlyMember(msg.sender) returns (uint) {
    ...
    function executeProposal(
        uint256 proposalId
    ) external payable onlyMember(msg.sender) returns (uint256) {
        return
            super.execute(
    ...
    function getPropsalIds()
        external
        view
        returns (address[] memory idPropsals)
    {
    ...
    function getVotes(
        uint propsalId
    ) external view returns (uint against, uint fors) {}
}
