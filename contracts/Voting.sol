    ...
    GovernorVotesQuorumFraction
{
    ...
    address[] propsalIds; // массив из всех адресов
    ...
    // делегировать токены
    function delegateRTK(address account) external {
    ...
    function calculatePower (
        ...
        uint rtkP = rtkCoin.getVotes(account) / rtkPower;
        return profiP + rtkP;
    }
    ...
    function createProposal(
        ...
        emit CreatedProposal(ID, msg.sender, target, value, description, block.timestamp + delay, block.timestamp + delay + period, proposalType);
        return ID;
    }
    ...
    function getPropsalIds() public view returns (address[] memory idPropsals) {
        idPropsals = propsalIds;
    }
    ...
    function getVotes(
        uint propsalId
    ) public view returns (uint against, uint fors) {}
}
